...
namespace Netezos.Rpc.Queries.Post
{
    ...
    public class InjectBlockQuery : RpcMethod
    {
        ...
        /// <returns></returns>
        public IEnumerator PostAsync(string data, List<List<object>> operations,bool async = false, bool force = false, Chain chain = Chain.Main)
            => Client.PostJson(
        ...
        /// <returns></returns>
        public IEnumerator PostAsync<T>(string data, List<List<object>> operations, bool async = false, bool force = false, Chain chain = Chain.Main)
            => Client.PostJson<T>(
                $"{Query}?async={async}&force={force}&chain={chain.ToString().ToLower()}",
                new { data, operations });
    }
