...
namespace Netezos.Rpc.Queries.Post
{
    ...
    public class BigMapQuery : RpcMethod
    {
        ...
        /// <returns></returns>
        public IEnumerator PostAsync(object key, string type, string prim)
            => PostAsync(new
            {
        ...
        /// <param name="key">Key (micheline michelson expression)</param>
        /// <param name="type">Type of the key (micheline michelson expression)</param>
        public IEnumerator PostAsync(object key, object type)
            => PostAsync(new
            {
                key,
                type
        ...
        /// <param name="prim">Primitive michelson type</param>
        /// <returns></returns>
        public IEnumerator PostAsync<T>(object key, string type, string prim)
            => PostAsync<T>(new
            {
                key = new Dictionary<string, object> { { type, key } },
                type = new { prim }
        ...
        /// <param name="type">Type of the key (micheline michelson expression)</param>
        /// <returns></returns>
        public IEnumerator PostAsync<T>(object key, object type)
            => PostAsync<T>(new
            {
                key,
                type
